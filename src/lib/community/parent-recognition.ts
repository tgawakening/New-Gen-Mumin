...
export async function awardParentRecognition(input: { parentId: string; badgeKey: string; evidence: string; awardedByUserId: string; sourceId: string; featuredWeek?: string }) {
  ...
  let award;
  try {
    award = await db.parentRecognitionAward.create({ data: { parentId: input.parentId, badgeKey: badge.key, title: badge.title, description: badge.description, evidence: input.evidence, awardedByUserId: input.awardedByUserId, sourceId: input.sourceId, featuredWeek: input.featuredWeek } });
  } catch (error) {
  ...
  if (parent) {
    const name = `${parent.user.firstName} ${parent.user.lastName ?? ""}`.trim() || parent.user.email;
