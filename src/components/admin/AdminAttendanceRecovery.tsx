...
export function AdminAttendanceRecovery({ learners, today }: { learners: Learner[]; today: string }) {
  ...
  function defaultChoice(id:string):Choice{return {mode:'all',missedCount:0,missedKeys:[],teacherId:'',parentName:learners.find(s=>s.id===id)?.parents||''};}
  ...
  return <div className="space-y-5"><fieldset disabled={busy} className="space-y-5 disabled:opacity-70">
    ...
    <label className="block">Parent report / evidence (required)<textarea className="mt-1 block w-full rounded border p-2" rows={3} maxLength={4000} value={note} onChange={e=>{setNote(e.target.value);setConfirmed(false);}} placeholder="Example: Nida reports Mustafa attended every class from 1 August; July is uncertain and excluded."/></label>
    ...
    {ids.map(id=>{const data=rangePreviewed===rangeKey?previews[id]:undefined,choice=choices[id]??defaultChoice(id);const teachers=new Map<string,string>();data?.sessions.forEach(s=>s.teacherIds.forEach((tid,i)=>teachers.set(tid,s.teachers[i]??s.teachers.join(', '))));const missed=choice.mode==='count'?choice.missedCount:choice.mode==='dates'?choice.missedKeys.length:0;return <section key={id} className="space-y-3 rounded-xl border bg-white p-4"><h2 className="text-lg font-bold">{learners.find(s=>s.id===id)?.name}</h2>
      ...
      <label className="block">Reporting parent (required)<input className="ml-2 rounded border p-2" value={choice.parentName} onChange={e=>change(id,{parentName:e.target.value})}/></label>
