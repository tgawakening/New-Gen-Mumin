...
export function MobileTeacherNavRailClient({ navItems }: { navItems: NavItem[] }) {
  ...
  return <div className="mt-5 rounded-[22px] border border-white/10 bg-white/[0.06] p-3">
    ...
    <nav className="grid grid-cols-2 gap-2 sm:flex sm:flex-wrap" aria-label="Priority teaching sections">{primary.map((item)=><TeacherNavLinkClient key={item.href} {...item} variant="mobileTab"/>)}</nav>
    {expanded && more.length?<div className="mt-3 border-t border-white/10 pt-3"><p className="mb-2 px-1 text-xs font-semibold text-white/70">All teaching sections</p><nav className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-5">{more.map((item)=><TeacherNavLinkClient key={item.href} {...item} variant="mobileTab"/>)}</nav></div>:null}
  </div>;
}
