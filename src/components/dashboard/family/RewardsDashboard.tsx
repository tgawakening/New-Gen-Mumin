  ...
  return <div className="space-y-5">
    <section className="relative overflow-hidden rounded-[28px] border border-[#eadfce] bg-gradient-to-br from-white via-white to-[#fff2dc] p-5 shadow-sm lg:min-h-64 lg:pr-64">
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        <span className="rounded-full border border-[#d8e3ed] bg-[#fbf6ef] px-4 py-2 text-sm font-semibold text-[#22304a]">{data.membership.qabilaGroup || "Qabila assignment pending"}{data.membership.role !== "MEMBER" ? ` — ${data.membership.role.replace("_", " ")}` : ""}</span>
      </div>
      <div className="absolute right-8 top-5 z-20 hidden max-w-44 rounded-[22px] border border-[#f1c878] bg-white px-4 py-3 text-center text-sm font-bold text-[#22304a] shadow-lg lg:block">{data.nextUnlock ? `Only ${Math.max(0, data.nextUnlock.milestone - data.collective)} points to the next Qabila unlock!` : "Your Qabila unlocked every published reward!"}</div>
