    ...
    <div className="space-y-5">
      ...
      <section id="weekly-certificate" className="scroll-mt-6 grid gap-5 xl:grid-cols-[1fr_1.08fr]">
        ...
        <div className="rounded-[30px] border border-[#dca142] bg-[#11294a] p-3 shadow-[0_18px_45px_rgba(17,41,74,0.2)]">
          <div className="relative min-h-[330px] overflow-hidden rounded-[22px] border-2 border-[#e5b85f] bg-[#fffaf1] p-5">
            ...
            <div className="relative mt-4 grid min-h-[195px] grid-cols-[120px_1fr] items-center gap-4 sm:grid-cols-[150px_1fr]">
              <div className="relative h-[190px] overflow-hidden rounded-[24px] border-2 border-[#edc36f] bg-[#fff3d4]"><Character data={data} className="absolute inset-0 h-full w-full"/></div>
              <div className="text-center"><p className="text-xs font-semibold text-[#6d7785]">Proudly presented to</p><h3 className="mt-1 text-3xl font-black text-[#14233e]">{name}</h3><p className="mt-2 font-serif text-xl font-bold uppercase tracking-[.1em] text-[#bd6e1d]">{spotlightTitle}</p><div className="mx-auto mt-3 inline-flex items-center gap-2 rounded-full bg-[#e9f7ee] px-3 py-1.5 text-[10px] font-black uppercase tracking-[.1em] text-[#27734b]"><Shield className="h-3.5 w-3.5"/>{featured?.category ?? "Character growth"}</div><p className="mx-auto mt-3 line-clamp-3 max-w-md text-xs leading-5 text-[#596779]">{spotlightEvidence}</p>{featured ? <Link href={`/certificates/${featured.certificateCode}`} className="mt-4 inline-flex items-center gap-2 rounded-full bg-[#11294a] px-5 py-2.5 text-xs font-bold text-white shadow-lg transition hover:-translate-y-0.5"><Download className="h-4 w-4"/>Open, print or download</Link> : <span className="mt-4 inline-flex rounded-full border border-[#d7c5aa] px-4 py-2 text-xs font-bold text-[#7c6a50]">Earn a badge to unlock</span>}</div>
            </div>
