...
export function QabilaMessageComposer({ action, roomId, studentId, mentions, replyTo, buttonLabel = "Send message" }: { action: (formData: FormData) => void | Promise<void>; roomId: string; studentId?: string; mentions: Option[]; replyTo?: Option | null; buttonLabel?: string }) {
  ...
  return <form action={action} className="rounded-b-[24px] border-t border-[#dbe3ec] bg-white p-3 sm:p-4">
    <input type="hidden" name="roomId" value={roomId}/>{studentId ? <input type="hidden" name="studentId" value={studentId}/> : null}{replyTo ? <input type="hidden" name="replyToId" value={replyTo.id}/> : null}
    ...
    <select name="mentionName" className="mb-2 w-full rounded-xl border border-[#d8e3ed] bg-[#f7f9fb] px-3 py-2 text-xs text-[#4d5a6b]"><option value="">Tag a teammate or teacher...</option>{mentions.map((item)=><option key={item.id} value={item.label}>@{item.label}</option>)}</select>
    <textarea id="qabila-composer" name="body" value={body} onChange={(event)=>setBody(event.target.value)} required maxLength={800} rows={2} placeholder="Write a friendly message..." className="w-full resize-none rounded-2xl border border-[#cfdbe7] px-4 py-3 text-sm text-[#22304a] outline-none focus:border-[#2a76aa]"/>
