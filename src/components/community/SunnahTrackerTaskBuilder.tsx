...
export function SunnahTrackerTaskBuilder({ initialTasks }: { initialTasks?: Task[] }) {
  ...
  return <div className="grid gap-4">
    ...
    {tasks.map((task, index) => <div key={task.id ?? index} className="rounded-[22px] border border-[#d8e3ed] bg-[#f8fbff] p-3 sm:p-4">
      {task.id ? <input type="hidden" name="taskId" value={task.id}/> : null}<input type="hidden" name="taskIcon" value={task.iconKey}/>
      <div className="flex gap-3"><span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-[#102544] text-sm font-black text-[#ffc96c]">{index + 1}</span><input name="taskPrompt" value={task.prompt} onChange={(event) => update(index, { prompt: event.target.value })} required placeholder="Write a Sunnah task" className="min-w-0 flex-1 rounded-xl border border-[#cfdbe7] bg-white px-3 py-2 text-sm"/><button type="button" onClick={() => setTasks((current) => current.filter((_, taskIndex) => taskIndex !== index))} className="rounded-xl px-2 text-[#b24646]" aria-label="Remove task"><Trash2 className="h-4 w-4"/></button></div>
