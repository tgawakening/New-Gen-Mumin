...
export default async function Page(){
 ...
 return <div className="mx-auto max-w-6xl space-y-6 p-6"><Link href="/admin" className="underline">Back to admin dashboard</Link><h1 className="text-3xl font-bold">Attendance recovery</h1><p>Record parent-reported attendance for previous months. August is selected by default; adjust the dates as needed.</p><AdminAttendanceRecovery learners={[...names.values()].sort((a,b)=>a.name.localeCompare(b.name))} today={attendanceDayKey(new Date())}/><section className="space-y-3"><h2 className="text-xl font-bold">Recent recovery reports</h2>{reports.map(report=><details key={report.id} className="rounded border bg-white p-3"><summary>{report.student.displayName} - {report.fromDay} to {report.toDay} - {report.parentName}</summary><p>{report.note}</p><p>{report.missedCount} missed with unknown dates. To revise, preview this learner using the same date range.</p><ul className="mt-2 space-y-2 text-sm">{(Array.isArray(report.revisions)?report.revisions:[]).map((value,index)=>{const entry=value as { savedAt?: string; parentName?: string; mode?: string; missedCount?: number; pointsDelta?: number; note?: string };return <li key={index} className="rounded bg-slate-50 p-2">Saved {entry.savedAt ? new Date(entry.savedAt).toLocaleString("en-GB",{timeZone:"Asia/Karachi"}) : ""} PKT | {entry.parentName} | {entry.mode==="all"?"Attended all":entry.mode==="count"?`${entry.missedCount} missed; dates unknown`:"Specific absences recorded"} | Points adjustment: {entry.pointsDelta}<p>{entry.note}</p></li>;})}</ul></details>)}</section></div>;
}
