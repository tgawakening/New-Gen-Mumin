...
function RevenueBreakdown({
  ...
  items: Array<{ gateway: string; totalGbp: number; orderCount: number }>;
}) {
  const maxTotal = Math.max(1, ...items.map((item) => item.totalGbp));

  return (
    <section className="rounded-[28px] border border-[#dce4ed] bg-white p-6 shadow-sm">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.2em] text-[#c27a2c]">Payment records</p>
          <h2 className="mt-2 text-2xl font-semibold text-[#22304a]">Revenue by payment method</h2>
          <p className="mt-2 max-w-2xl text-sm leading-7 text-[#617184]">
            Completed Stripe, PayPal, and bank transfer orders are converted to GBP using the site pricing rates.
          </p>
        </div>
        <Link
      ...
      <div className="mt-5 grid gap-4 lg:grid-cols-3">
        {items.map((item) => {
          const width = Math.max(8, Math.round((item.totalGbp / maxTotal) * 100));
          return (
            <div key={item.gateway} className="rounded-[22px] border border-[#e6edf4] bg-[#fbfdff] p-4">
              <div className="flex items-center justify-between gap-3">
                <p className="font-semibold text-[#22304a]">{paymentMethodLabel(item.gateway)}</p>
                <span className="rounded-full bg-[#eef6ff] px-3 py-1 text-xs font-semibold text-[#0f4d81]">
                  {item.orderCount} orders
                </span>
              </div>
              <p className="mt-3 text-2xl font-semibold text-[#22304a]">{formatMoney(item.totalGbp, "GBP")}</p>
              <div className="mt-4 h-3 overflow-hidden rounded-full bg-[#e8eef5]">
                <div className="h-full rounded-full bg-[#0f4d81]" style={{ width: `${width}%` }} />
              </div>
            </div>
          );
        })}
