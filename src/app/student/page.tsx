  ...
  return (
    ...
    >
      ...
      <section className="overflow-hidden rounded-[30px] border bg-white shadow-sm" style={{ borderColor: qabila?.color ?? "#d8e3ed" }}>
        ...
        {qabila ? <div className="border-t border-[#eadfce] px-5 py-4"><div className="flex flex-wrap items-center justify-between gap-2"><div><p className="text-sm font-semibold text-[#22304a]">My Qabila teammates</p><p className="mt-1 text-xs text-[#617184]">Get to know the learners growing alongside you.</p></div><Link href="/student/community" className="text-xs font-semibold text-[#0f4d81]">See team and chat →</Link></div><div className="mt-3 flex flex-wrap gap-2"><span className="rounded-full px-3 py-2 text-xs font-semibold text-white" style={{ backgroundColor: qabila.color }}>{dashboard.studentName} (You)</span>{teammates.slice(0,18).map((member)=><span key={member.id} className="rounded-full border border-[#eadfce] bg-[#fffaf4] px-3 py-2 text-xs font-semibold text-[#22304a]">{member.name}</span>)}</div>{!teammates.length ? <p className="mt-3 text-xs text-[#617184]">Your teammates will appear here as their registrations are confirmed.</p> : null}</div> : null}
      </section>
