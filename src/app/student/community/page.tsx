    ...
    >
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1.2fr)_320px]">
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-3">
                  {[...membership.room.messages].reverse().map((message) => (
                    <div data-reply-href={"/student/community?reply=" + message.id + "#qabila-composer"} id={`message-${message.id}`} key={message.id} className={`rounded-[18px] border p-4 text-sm ${message.author.role === "TEACHER" ? "border-[#efbd68] bg-[#fff4d8] shadow-sm" : "border-transparent bg-[#fbf6ef]"}`}>
                      ...
                      </div>                      <p className="mt-2 whitespace-pre-wrap leading-6 text-[#4d5a6b]">{message.body}</p><CommunityMessageMedia message={message} />
        ...
        <div className="space-y-5 xl:sticky xl:top-4 xl:self-start">
          ...
          <SectionCard eyebrow="Safety rules" title="Community guardrails" icon="check"><CompactList items={[{ label: "Keep personal details private", meta: "Do not share phone numbers or emails", icon: "check" }, { label: "Mentor and admin moderation", meta: "Messages publish normally and can be reviewed", icon: "check" }, { label: "No uncontrolled private chat", meta: "Room-based MVP", icon: "check" }, { label: "Mentor-visible logs", meta: "Designed for trust", icon: "star" }]} emptyLabel="Safety rules will appear here." /></SectionCard>
        </div>
      </div>
    </FamilyDashboardFrame>
  );
