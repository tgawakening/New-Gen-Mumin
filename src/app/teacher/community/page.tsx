...
export default async function TeacherCommunityPage({ searchParams }: PageProps) {
  ...
  async function postMessage(formData: FormData) {
    ...
    try {
      await postTeacherCommunityMessage({ userId: current.user.id, roomId, body, mentionedUserId: String(formData.get("mentionedUserId") || "") || null });
    } catch (error) {
