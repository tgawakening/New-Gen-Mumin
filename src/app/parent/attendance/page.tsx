...
export default async function ParentAttendancePage({ searchParams }: PageProps) {
  ...
  return <FamilyDashboardFrame roleLabel="Parent Dashboard" title="Attendance" subtitle="Review each child's session-by-session attendance and verified Zoom time." navItems={getParentNavItems(child.id)} pendingReason={dashboard.pendingReason}><SectionCard eyebrow="Learner" title="Choose a child"><ChildSelector learners={dashboard.children.map((item) => ({ id: item.id, name: item.name }))} selectedChildId={child.id} basePath="/parent/attendance" /></SectionCard><MetricGrid metrics={[{ label: "Sessions", value: String(history.length), hint: "Recorded classes." }, { label: "Attended", value: String(attended), hint: "Present or late." }, { label: "Class time", value: `${minutes} min`, hint: "Verified Zoom time." }, { label: "Attendance", value: `${child.attendanceRate}%`, hint: "Current attendance rate." }]} /><SectionCard eyebrow="Session history" title={`${child.name}'s recent attendance`}><p className="mb-4 text-sm text-[#617184]">Seerah and Life Skills each count once per Pakistan calendar day. Attend either time slot; present and late arrivals both count as attended. Connection time excludes recorded disconnect gaps.</p><AttendanceHistory records={history} /></SectionCard></FamilyDashboardFrame>;
}
