          ...
          {visibleMemberships.map((membership) => (
            <SectionCard key={membership.id} eyebrow={membership.room.type.replace(/_/g, " ")} title={membership.room.title} icon="sun">
              <div className="space-y-3">
                ...
                <div className="max-h-[620px] space-y-3 overflow-y-auto overscroll-contain scroll-smooth p-1">
                <QabilaChatEnhancer />
                {[...membership.room.messages].reverse().map((message) => (
                  <div data-reply-href={"/parent/community?child=" + selectedChildId + "&room=" + membership.room.id + "&reply=" + message.id + "#qabila-composer"} id={`message-${message.id}`} key={message.id} className={`rounded-[18px] border p-4 text-sm ${message.author.role === "TEACHER" ? "border-[#efbd68] bg-[#fff4d8] shadow-sm" : "border-transparent bg-[#fbf6ef]"}`}>
                    ...
                    <p className="mt-2 whitespace-pre-wrap leading-6 text-[#4d5a6b]">{message.body}</p><CommunityMessageMedia message={message} />
